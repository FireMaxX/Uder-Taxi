...
GtkWidget *CreateWindow(int argc, char *argv[])
{
	...
	gtk_window_set_title(GTK_WINDOW(Window), "Uder V1.0");
	gtk_window_set_default_size(GTK_WINDOW(Window), GTKMaplength, GTKMapwidth + 20);
	...
	gtk_widget_set_size_request(DrawArea, GTKMaplength, GTKMapwidth);
	...
	frame = gtk_frame_new("Latest Notice");
	label = gtk_label_new(GTKbuffer);
	gtk_container_add(GTK_CONTAINER(frame), label);
	gtk_table_attach_defaults(GTK_TABLE(table), frame, 0, 5, 0, 10);

	GTKMessage = (GtkLabel*)label;
	GTKMap = DrawArea;
...
void DriveCar(cairo_t *cr, Point taxi)
{
	...
	cairo_arc(cr, taxi.y + 1, taxi.x + 1, 0.5, 0, 2 * M_PI);
	cairo_fill(cr);
